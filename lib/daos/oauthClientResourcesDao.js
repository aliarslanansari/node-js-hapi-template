...
import { oauth_client_resources as oauthClientResources } from 'models';
import { hasPowerOver, getScope } from 'utils';
...
export const findClientResources = (token, page, limit) => {
    ...
    return oauthClientResources.findAll({
        attributes: ALL_ATTRIBUTES,
        where: {
            oauthClientId
        },
...
export const createResource = async (resource, token) => {
    ...
    if (hasPowerOver(token, resource.oauthClientId, scope)) {
        return oauthClientResources.create({
            ...resource,
    ...
    throw unauthorized(
        `User with clientId: ${
            token.oauthClientId
        } is not authorized to create resources.`
    );
};
...
export const updateResource = async (resource, token) => {
    ...
    if (hasPowerOver(token, resource.oauthClientId, scope)) {
                    ...
                    .update(
                        {
                            ...
                            resourceId:
                                resource.resourceId || foundResource.resourceId
                        },
                        {
                            where: {
                                id: resource.id,
        ...
        throw badRequest(
            `Unable to update resource with resourceId: ${
                resource.id
            }, oauthClientId: ${resource.oauthClientId}`
        );
    }
    throw unauthorized(
        `User with clientId: ${
            token.oauthClientId
        } is not authorized to update this resource.`
    );
};
...
export const createOauthResources = ({ oauthClientId, resources }, t) =>
    oauthClientResources.bulkCreate(
